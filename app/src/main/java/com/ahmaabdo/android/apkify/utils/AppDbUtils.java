...
//SQL Database to save the loaded apps
public class AppDbUtils extends SQLiteOpenHelper {
    //TODO: rewrite AppDbUtils
    ...
    public void removeAppInfo(AppInfo appInfo) {
        database.delete(
                ...
                COLUMN_NAME_APK + " = ?",
                new String[]{
                        String.valueOf(appInfo.getAPK())
                });
    }

    public Boolean checkAppInfo(AppInfo appInfo, int data) {
        String QUERY_EXIST =
                QUERY + " WHERE " + COLUMN_NAME_APK + " = '" + appInfo.getAPK() + "'";
