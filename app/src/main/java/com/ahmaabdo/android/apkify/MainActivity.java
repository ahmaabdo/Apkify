...
public class MainActivity extends ThemeActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        drawer = InterfaceUtils.setNavigationDrawer(
                (Activity) context, context, toolbar, appAdapter, appSystemAdapter, appFavoriteAdapter,
                appHiddenAdapter, appDisabledAdapter, recyclerView);
        ...
        protected void onPostExecute(Void voids) {
            switch (App.getCurrentAdapter()) {
                default:
                    ...
                    InterfaceUtils.setToolbarTitle(activity,
                            getResources().getString(R.string.action_installed_apps));
                    break;
                case 1:
        ...
        switch (requestCode) {
            case MY_PERMISSIONS_REQUEST_WRITE_READ: {
                if (grantResults[0] != PackageManager.PERMISSION_GRANTED) {
                    DialogUtils.showTitleContent(
                            context,
                            getResources().getString(R.string.dialog_permissions),
